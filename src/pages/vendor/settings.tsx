...
const VendorSettings = () => {
    ...
    const { data: settings, refetch } = trpc.useQuery(['vendor.settings.get'], {
    ...
    const { mutateAsync: createSettingsAsync } = trpc.useMutation(['vendor.settings.create']);
    ...
    const onSettingsSave = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!settings) {
            const { id } = await createSettingsAsync({
                paypalEmail,
